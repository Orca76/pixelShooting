...
public class GunManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        nameText = Players.GetComponent<PlayerBase>().gunNameTextObj;
        mpText = Players.GetComponent<PlayerBase>().gunMpText;
        damageText = Players.GetComponent<PlayerBase>().gunDamageText;
        explainText = Players.GetComponent<PlayerBase>().gunExplainText;

        turret = GameObject.Find("ShotPos");
    ...
    void Update()
    {
        ...
        if (!Players.GetComponent<PlayerBase>().isGameOver)//ゲームオーバーになっていない
        {
            ...
            {
               Debug.Log("GunManagerに来ました　今から次のキャラの番号を決めマス");
                int i = 1;
                while (true)
                {
            ...
            for (int j = 0; j < 4; j++)
            {
                ...
                if (exist[j] == true)//プレイヤーは存在していて
                {
                    ...
                    if (PlayerExistObj[j] == null)//スキルは無い
                    {
                        ...
                        Light2D spriteLight = charaSprite.AddComponent<Light2D>();
                        spriteLight.lightType = Light2D.LightType.Sprite;
                        spriteLight.lightCookieSprite = Players.GetComponent<PlayerBase>().charactorImage[createNumber];//スプライトを登録
    ...
    void Shot()//銃弾を撃つ
    {
        if (ActiveCharactorIndex == 0)//4で割ったあまりで判別
        {
            if (Components0[0] != null)
            {
               ...
               , turret.transform.position, turret.transform.rotation);
